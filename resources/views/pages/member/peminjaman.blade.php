                    ...
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-3xl font-bold text-green-600">
                                {{ $pinjaman->whereIn('status', ['dipinjam', 'pending'])->count() }}</p>
                        </div>
                    ...
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-3xl font-bold text-purple-600">
                                {{ $pinjaman->where('status', 'dikembalikan')->count() }}</p>
                        </div>
                                ...
                                <div class="flex flex-col lg:flex-row gap-6">
                                    ...
                                    <div class="flex gap-4 lg:flex-1">
                                        <a href="{{ route('buku', $p->buku->slug) }}" class="flex-shrink-0">
                                            <img src="{{ $p->buku->bannerUrl ?: asset('images/media_placeholder.webp') }}"
                                                alt="{{ $p->buku->judul }}"
                                        ...
                                        <div class="flex-1 min-w-0">
                                            <a href="{{ route('buku', $p->buku->slug) }}" class="group">
                                                <h3
                                                    class="text-lg font-bold text-gray-900 group-hover:text-indigo-600 transition-colors line-clamp-2">
                                                    {{ $p->buku->judul }}
                                                </h3>
                                            </a>
                                            <p class="text-sm text-gray-500 mt-1">oleh
                                                {{ $p->buku->author ?? 'Tidak Diketahui' }}</p>
                                    ...
                                    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 lg:flex-1">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-sm font-semibold text-gray-900">
                                                {{ \Carbon\Carbon::parse($p->loan_date)->format('d M Y') }}</p>
                                        </div>
                                        ...
                                        <div>
                                            ...
                                            <p class="text-sm font-semibold text-gray-900">
                                                {{ \Carbon\Carbon::parse($p->due_date)->format('d M Y') }}</p>
                                        </div>
                                    ...
                                    <div class="lg:w-48 flex flex-col justify-center gap-3" x-data="{ showWarning: false, loading: false, extended: {{ $p->extended ? 'true' : 'false' }} }">
                                        ...
                                        @if ($p->status === 'dipinjam' && !$p->extended)
                                            <button x-show="!extended" :disabled="loading"
                                                @click="
                                            ...
                                            <span x-show="extended"
                                                class="text-xs text-green-600 italic text-center font-semibold">✓ Sudah
                                                diperpanjang</span>
                                        @elseif($p->extended)
                                            <span class="text-xs text-gray-400 italic text-center">Sudah
                                                diperpanjang</span>
                                        @endif
                ...
                @else
                    ...
                    <div class="text-center py-16 px-4">
                        ...
                        <a href="{{ route('allBuku') }}"
                            class="inline-flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-xl font-semibold hover:from-indigo-700 hover:to-purple-700 transition-all shadow-lg hover:shadow-xl">
