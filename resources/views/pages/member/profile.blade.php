    ...
    <section class="py-12 bg-white -mt-20 relative z-20">
        <div class="container mx-auto px-4">
            ...
            <div class="bg-white rounded-3xl shadow-xl border border-gray-100 overflow-hidden mb-8" x-data="{ showPhotoModal: false }">
                    ...
                    <div
                        ...
                        <div class="relative group">
                            <div class="w-48 h-64 lg:w-56 lg:h-72 rounded-2xl overflow-hidden shadow-2xl ring-4 ring-white">
                                <img src="{{ Storage::disk('s3')->url($member->image) ?? asset('users/images/profile-placeholder.png') }}"
                    ...
                    <div class="flex-1 p-8 lg:p-10">
                        <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-6">
                            <div>
                                ...
                                <div class="flex items-center gap-3 mt-2">
                                    <span
                                        class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-indigo-100 text-indigo-700 uppercase">
                                    ...
                                    @else
                                        <span
                                            class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-yellow-100 text-yellow-700">
                        ...
                        <div class="grid sm:grid-cols-2 gap-4 mb-6">
                            ...
                            <div class="bg-gray-50 rounded-2xl p-4">
                                ...
                                <p class="text-sm font-semibold text-gray-900 font-mono">{{ $member->membership_number }}
                                </p>
                            </div>
                            <div class="bg-gray-50 rounded-2xl p-4">
                                <p class="text-xs font-medium text-gray-500 mb-1 uppercase tracking-wider">Bergabung</p>
                                <p class="text-sm font-semibold text-gray-900">
                                    {{ $member->user->created_at->format('d M Y') }}</p>
                            </div>
                            <div class="bg-gray-50 rounded-2xl p-4">
                                <p class="text-xs font-medium text-gray-500 mb-1 uppercase tracking-wider">Terakhir Login
                                </p>
                                <p class="text-sm font-semibold text-gray-900">
                                    {{ $member->user->last_login_at?->diffForHumans() ?? '-' }}</p>
                            </div>
                        </div>
                ...
                <div x-show="showPhotoModal" x-cloak style="display:none;" class="fixed inset-0 z-50 overflow-y-auto"
                    @keydown.escape.window="showPhotoModal = false">
                    <div class="fixed inset-0 bg-black/50 backdrop-blur-sm" @click="showPhotoModal = false"
                        x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0"
                        x-transition:enter-end="opacity-100" x-transition:leave="ease-in duration-200"
                        x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0"></div>
                    <div class="flex min-h-screen items-center justify-center p-4">
                        <div x-show="showPhotoModal" x-transition:enter="ease-out duration-300"
                            ...
                            <form id="cropForm" action="{{ route('member.updatePhoto') }}" method="POST">
                                ...
                                <div class="bg-gray-50 px-6 py-4 flex gap-3 justify-end">
                                    ...
                                    <button type="submit"
                                        class="px-6 py-2.5 rounded-xl bg-gradient-to-r from-indigo-600 to-purple-600 text-white font-semibold hover:from-indigo-700 hover:to-purple-700 transition-all shadow-lg">
                                        Simpan Foto
                                    </button>
                                </div>
                            </form>
            ...
            @php
                ...
                $activeLoans = \App\Models\Pinjaman::where('member_id', $member->membership_number)
                    ->whereIn('status', ['dipinjam', 'menunggu_verif'])
                    ->count();
                $returnedBooks = \App\Models\Pinjaman::where('member_id', $member->membership_number)
                    ->where('status', 'dikembalikan')
                    ->count();
                $reviewCount = \App\Models\BookReview::where('member_id', $member->membership_number)->count();
                $reservationCount = \App\Models\BookReservation::where('member_id', $member->membership_number)
            ...
            <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-8">
                <div
                    class="bg-gradient-to-br from-indigo-500 to-purple-600 rounded-3xl p-5 text-white text-center shadow-lg">
                ...
                <div
                    class="bg-gradient-to-br from-emerald-500 to-green-600 rounded-3xl p-5 text-white text-center shadow-lg">
                    <p class="text-3xl font-bold">{{ $reviewCount }}</p>
                    <p class="text-xs font-medium text-white/80 mt-1">Ulasan</p>
                </div>
                <div
                    class="bg-gradient-to-br from-amber-500 to-orange-600 rounded-3xl p-5 text-white text-center shadow-lg">
                    <p class="text-3xl font-bold">{{ $reservationCount }}</p>
                    <p class="text-xs font-medium text-white/80 mt-1">Reservasi</p>
                </div>
                <div
                    class="bg-gradient-to-br from-violet-500 to-fuchsia-600 rounded-3xl p-5 text-white text-center shadow-lg">
            ...
            <div class="grid grid-cols-2 md:grid-cols-5 gap-4 mb-8">
                <a href="{{ route('pinjam') }}"
                    class="group bg-white rounded-2xl p-5 border border-gray-100 shadow-sm hover:shadow-lg hover:border-indigo-200 transition-all duration-300 hover:-translate-y-1">
                    <div class="flex items-center gap-4">
                        <div
                            class="w-12 h-12 rounded-2xl bg-indigo-100 flex items-center justify-center group-hover:bg-indigo-200 transition-colors">
                ...
                <a href="{{ route('readingHistory') }}"
                    class="group bg-white rounded-2xl p-5 border border-gray-100 shadow-sm hover:shadow-lg hover:border-green-200 transition-all duration-300 hover:-translate-y-1">
                    <div class="flex items-center gap-4">
                        <div
                            class="w-12 h-12 rounded-2xl bg-green-100 flex items-center justify-center group-hover:bg-green-200 transition-colors">
                ...
                <a href="{{ route('wishlist.index') }}"
                    class="group bg-white rounded-2xl p-5 border border-gray-100 shadow-sm hover:shadow-lg hover:border-pink-200 transition-all duration-300 hover:-translate-y-1">
                    <div class="flex items-center gap-4">
                        <div
                            class="w-12 h-12 rounded-2xl bg-pink-100 flex items-center justify-center group-hover:bg-pink-200 transition-colors relative">
                            ...
                            @if ($member->wishlist->count() > 0)
                                <span
                                    class="absolute -top-1 -right-1 w-5 h-5 bg-pink-500 text-white text-xs font-bold rounded-full flex items-center justify-center">{{ $member->wishlist->count() }}</span>
                            @endif
                        </div>
                ...
                <a href="{{ route('reservations') }}"
                    class="group bg-white rounded-2xl p-5 border border-gray-100 shadow-sm hover:shadow-lg hover:border-amber-200 transition-all duration-300 hover:-translate-y-1">
                    <div class="flex items-center gap-4">
                        <div
                            class="w-12 h-12 rounded-2xl bg-amber-100 flex items-center justify-center group-hover:bg-amber-200 transition-colors">
                ...
                <a href="{{ route('notifications') }}"
                    class="group bg-white rounded-2xl p-5 border border-gray-100 shadow-sm hover:shadow-lg hover:border-purple-200 transition-all duration-300 hover:-translate-y-1">
                    <div class="flex items-center gap-4">
                        <div
                            class="w-12 h-12 rounded-2xl bg-purple-100 flex items-center justify-center group-hover:bg-purple-200 transition-colors">
            ...
            <div class="bg-white rounded-3xl shadow-xl border border-gray-100 overflow-hidden">
                <div class="bg-gradient-to-r from-indigo-600 to-purple-600 px-8 py-6">
                    <div class="flex items-center justify-between">
                        ...
                        <a href="{{ route('pinjam') }}"
                            class="inline-flex items-center gap-2 px-4 py-2 bg-white/20 backdrop-blur-sm text-white rounded-lg font-semibold hover:bg-white/30 transition-all border border-white/30 text-sm">
                            Lihat Semua
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <div class="p-5 hover:bg-gray-50 transition-colors">
                                <div class="flex items-center gap-4">
                                    ...
                                    <div class="flex-1 min-w-0">
                                        <a href="{{ route('buku', $loan->buku->slug) }}"
                                            class="font-semibold text-gray-900 hover:text-indigo-600 transition-colors text-sm line-clamp-1">{{ $loan->buku->judul }}</a>
                                    ...
                                    @endphp
                                    <span
                                        class="inline-flex px-3 py-1 rounded-full text-xs font-bold {{ $sColors[$loan->status] ?? 'bg-gray-100 text-gray-700' }}">
                ...
                @else
                    <div class="p-12 text-center">
                        ...
                        <a href="{{ route('allBuku') }}"
                            class="inline-flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-xl font-semibold hover:from-indigo-700 hover:to-purple-700 transition-all shadow-lg">
                            Jelajahi Buku
                        </a>
                    </div>
                @endif
