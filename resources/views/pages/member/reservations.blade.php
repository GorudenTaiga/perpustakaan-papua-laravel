...
@section('content')
    {{-- Hero Section --}}
    <section
        class="relative overflow-hidden bg-gradient-to-br from-amber-500 via-orange-500 to-red-500 py-20 lg:py-28 pb-32">
        <div
        ...
        <div class="container mx-auto px-4">
                ...
                <div class="bg-white rounded-3xl shadow-lg border border-gray-100 overflow-hidden">
                    ...
                    <div class="divide-y divide-gray-100">
                        @foreach ($reservations as $r)
                            <div
                                class="p-6 flex flex-col lg:flex-row gap-6 items-center hover:bg-gray-50 transition-colors">
                                <a href="{{ route('buku', $r->buku->slug) }}" class="flex-shrink-0">
                                    <img src="{{ $r->buku->bannerUrl ?: asset('images/media_placeholder.webp') }}"
                                        alt="{{ $r->buku->judul }}" class="w-16 h-24 object-cover rounded-xl shadow-md">
                                </a>
                                <div class="flex-1 min-w-0">
                                    ...
                                    <p class="text-xs text-gray-400 mt-1">Direservasi:
                                        {{ $r->created_at->format('d M Y H:i') }}</p>
                                </div>
                                <div class="flex items-center gap-4">
                                    ...
                                    @endphp
                                    <span
                                        class="px-4 py-2 rounded-full text-sm font-bold {{ $statusColors[$r->status] ?? 'bg-gray-100 text-gray-700' }}">
                                    ...
                                    @if ($r->status === 'waiting')
                                        <form action="{{ route('reservation.cancel') }}" method="POST">
                                            ...
                                            <button type="submit"
                                                class="px-4 py-2 rounded-xl border-2 border-red-200 text-red-600 font-semibold hover:bg-red-50 transition-colors text-sm">
                                                Batalkan
                                            </button>
                                        </form>
                                    @endif
            ...
            @else
                <div class="text-center py-16">
                    <div
                        class="inline-flex items-center justify-center w-20 h-20 rounded-full bg-gradient-to-br from-amber-50 to-orange-50 mb-6">
                    ...
                    <a href="{{ route('allBuku') }}"
                        class="inline-flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-amber-500 to-orange-500 text-white rounded-xl font-semibold hover:from-amber-600 hover:to-orange-600 transition-all shadow-lg">
                        Jelajahi Buku
                    </a>
                </div>
            @endif
