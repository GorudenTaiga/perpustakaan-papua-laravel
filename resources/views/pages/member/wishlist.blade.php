        ...
        <div class="container mx-auto px-4">
                                ...
                                <div class="flex gap-5 p-6">
                                    ...
                                    <a href="{{ route('buku', $item->buku->slug) }}" class="flex-shrink-0">
                                        <div class="relative w-24 h-36 overflow-hidden rounded-2xl shadow-md">
                                            <img src="{{ $item->buku->banner ? Storage::disk('public')->url($item->buku->banner) : asset('images/no-cover.png') }}"
                                                alt="{{ $item->buku->judul }}"
                                    ...
                                    <div class="flex-1 min-w-0 flex flex-col justify-between">
                                        <div>
                                            <a href="{{ route('buku', $item->buku->slug) }}" class="group/title">
                                                <h3
                                                    class="font-bold text-gray-900 text-base line-clamp-2 group-hover/title:text-indigo-600 transition-colors">
                                                    {{ $item->buku->judul }}
                                                </h3>
                                            </a>
                                            <p class="text-sm text-gray-500 mt-1">{{ $item->buku->author }}</p>

                                            {{-- Categories (using pre-loaded data to avoid N+1) --}}
                                            @if ($item->buku->relationLoaded('loadedCategories') && $item->buku->loadedCategories->count() > 0)
                                                <div class="flex flex-wrap gap-1.5 mt-2">
                                                    @foreach ($item->buku->loadedCategories->take(2) as $cat)
                                                        <span
                                                            class="px-2 py-0.5 rounded-full bg-indigo-50 text-indigo-600 text-xs font-medium">{{ $cat->nama_category }}</span>
                                                    @endforeach
                                                    @if ($item->buku->loadedCategories->count() > 2)
                                                        <span
                                                            class="px-2 py-0.5 rounded-full bg-gray-50 text-gray-500 text-xs">+{{ $item->buku->loadedCategories->count() - 2 }}</span>
                                                    @endif
                                                </div>
                                            @endif
                                            ...
                                            <div class="flex items-center gap-3 mt-2">
                                                @if ($item->buku->average_rating > 0)
                                                    <div class="flex items-center gap-1">
                                                        ...
                                                        <span
                                                            class="text-sm font-semibold text-gray-700">{{ number_format($item->buku->average_rating, 1) }}</span>
                                                    </div>
                                                @endif
                                                <div class="flex items-center gap-1">
                                                    <span
                                                        class="w-2 h-2 rounded-full {{ $item->buku->stok > 0 ? 'bg-green-400' : 'bg-red-400' }}"></span>
                                                    <span
                                                        class="text-xs {{ $item->buku->stok > 0 ? 'text-green-600' : 'text-red-600' }}">
            ...
            @else
                <div class="text-center py-20">
                    <div
                        class="inline-flex items-center justify-center w-24 h-24 rounded-full bg-gradient-to-br from-pink-50 to-rose-50 mb-6">
                    ...
                    <p class="text-gray-500 mb-8 max-w-md mx-auto">Anda belum menambahkan buku apapun ke wishlist. Jelajahi
                        koleksi kami dan temukan buku favorit Anda!</p>
                    <a href="{{ route('allBuku') }}"
                        class="inline-flex items-center gap-2 px-8 py-4 bg-gradient-to-r from-pink-500 to-rose-500 text-white rounded-2xl font-semibold hover:from-pink-600 hover:to-rose-600 transition-all shadow-lg hover:shadow-xl hover:-translate-y-0.5">
